package org.sisyphus.demo.kafka.ais;
...
/**
 * @Description
 * 1.kafka comuser消费kafka中的ais数据
 * 2.创建geomesa kafka DataStore
 * 3.将ais数据添加到geomesa kafka feature中
 */
public class Ais {
    ...
    public static void main(String[] args) throws Exception {
        ...
        final String sftName = "Ais";
        final String sftSchema = "mmsi:String,dtg:Date,*geom:Point:srid=4326";
...
//        props.put("auto.offset.reset","latest");
        ...
        // 拿数据
        List<String> topics = Arrays.asList("aistopic");
        ...
        while (true) {
            ...
            for (ConsumerRecord<String, String> record : records) {
                ...
                if (a.length >= 4) {
                    ...
                    builder.set("mmsi", a[0]);
                    builder.set("dtg", Date.from(LocalDateTime.parse(
                            a[1].replace("T", " "),
                            dateFormat).toInstant(ZoneOffset.UTC)));
...
//                    builder.set("dtg", Date.from(LocalDateTime.parse(a[1].replace("T", " "), dateFormat).toInstant(ZoneOffset.UTC)));
